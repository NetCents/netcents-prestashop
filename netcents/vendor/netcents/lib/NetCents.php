...
class NetCents
{
    ...
    public static function request($url, $method = 'POST', $params = array(), $authentication = array())
    {
        ...
        if (empty($auth_token))
            \NetCents\Exception::throwException(400, array('reason' => 'AuthTokenMissing'));
        ...
        if (!in_array($environment, $environments)) {
            ...
            \NetCents\Exception::throwException(400, array('reason' => 'BadEnvironment', 'message' => "Environment does not exist. Available environments: $availableEnvironments"));
        }

        $url       = ($environment === 'sandbox' ? 'https://api-sandbox.netcents.com/v2' : 'https://api.netcents.com/v2') . $url;
        $headers   = array();
